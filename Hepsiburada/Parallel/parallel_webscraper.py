...
def main():
  ...
  if rank<nprocs-1: # if slave
      url_index=rank
      while url_index<number_of_urls: # iterate thru all urls giving each processor as equal load as possible in a round-robin fashion 
        comm.send(save(urls[url_index]), tag=url_index, dest=nprocs-1)
        url_index+=nprocs-1
  else:
    # Load balance the urls across multiple CPUs
    offset=0
    for _ in range((number_of_urls-1)//(nprocs-1)+1): # iterate as many times as there should be round-robin like cycles
      for proc_index in range(nprocs-1):
        if proc_index+offset<number_of_urls:
          data = comm.recv(source=proc_index, tag=proc_index+offset)
          df = pd.DataFrame({'Ratings':data['ratings'],'Comments':data['comments']}) 
          df.to_csv('hepsiburada'+str(proc_index+offset)+'.csv', index=False, encoding='utf-8')
      offset+=nprocs-1  
